...
- Avoid caption strips or overlays; keep the layout as basic as possible.
...
- Optional, minimal branding (logo only) that never obscures critical content.

## Non-Goals
- Device mockups or 3D frames.
- Multi-image collages or carousels.
- Caption strips, CTA pills, or other overlay text bars.
...
- Full-bleed focus: prioritize the screenshot area with no overlays or caption strips.
...
- Safe branding: logo badge anchored outside the screenshot (top-left). No caption card.

## Layout & Behavior
- Canvas: 1280x720 export target; preview scales responsively in the playground.
- Screenshot container:
  - Max width ~92–96% of canvas; max height ~90% to preserve gutters.
  - Centers horizontally; vertical alignment can bias slightly upward if text is present elsewhere.
...
- Branding/text:
  ...
  - Title/description live in remaining free space (above, below, or aside the screenshot) with no strip/overlay.
- Empty state:
...
- Text: title + description positioning relative to screenshot (no caption strip overlays).
...
- Accessible: drop zones focusable and keyboard-invokable; text fields editable via keyboard.
...
- What are the preferred text anchor positions relative to the screenshot by default?
