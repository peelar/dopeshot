...
export function AuthForm() {
  ...
  const handleGoogleSignIn = (e: React.FormEvent<HTMLFormElement>) => {
    // Track attempt (fire and forget)
    track("auth_attempt", { method: "google" });
    // Let the form submit naturally - no preventDefault
    // Form submissions work reliably on mobile browsers for OAuth flows
  };
    ...
    <div className="mx-auto w-full max-w-md space-y-8 px-4">
      ...
      <div className="space-y-6 rounded-2xl border border-border/30 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:bg-background/60">
        {magicLinkSent ? (
          ...
          <>
            <form onSubmit={handleGoogleSignIn} action="/api/auth/sign-in/google" method="GET">
              <input type="hidden" name="callbackURL" value="/auth" />
              <button
                type="submit"
                className="flex h-12 w-full items-center justify-center rounded-lg border border-border/50 bg-muted/40 text-foreground transition-colors hover:bg-muted/60 hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
              >
