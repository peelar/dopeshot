...
The editor is fully client-side and ungated. Screenshots, testimonials, tweets, video preview, Brand panel, and catalog backgrounds are available without an account.

There is no login, subscription, or paywall in the product today.

## Direction

Screenshots remain the free hook. Brand consistency and additional formats are the likely paid product if/when accounts return.

## Why this split (when paid lands)
...
Each new format added to dopeshot makes a future paid tier more valuable without changing the price. A user paying for testimonials today automatically gets announcements tomorrow. The more formats behind the paywall, the stickier the subscription becomes.
...
- **No watermark on the current ungated product.** Watermarking screenshots would degrade the experience. Format gating is cleaner if a paywall returns.
